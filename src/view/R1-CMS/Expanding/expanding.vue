...
export default {
  ...
  methods: {
    ...
    getTableList () {
      let self = this
      self.$http(this.$config.BaseURL16 + '/custompromotion/template/list?page=' + self.page + '&pageSize=' + self.pageSize).then(response => {
    ...
    addOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/custompromotion/template/save', self.info).then(response => {
        if (response.data.errno == 10000) {
          this.info = {}
          this.getTableList()
          this.modalAdd = false
    ...
    editOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/custompromotion/template/save?id=' + self.id, self.edit).then(response => {
        if (response.data.errno == 10000) {
          this.getTableList()
    ...
    remove (row) {
      console.log(row)
      let self = this
      self.$http(this.$config.BaseURL16 + '/custompromotion/template/delete?id=' + row.id).then(response => {
        if (response.data.errno == 10000) {
          this.getTableList()
