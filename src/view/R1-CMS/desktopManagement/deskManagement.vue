...
export default {
  ...
  methods: {
    ...
    getTableList () {
      ...
      self.$http(this.$config.BaseURL16 + '/staticpromotion/desktop/list?templateId=' + self.search.templateId + '&status=' + self.search.status).then(response => {
    ...
    addOk () {
      console.log(111)
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/desktop/save', self.info).then(response => {
        console.log(response)
        if (response.data.errno == 10000) {
          this.info = {}
          this.modalAdd = false
    ...
    editOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/desktop/save?id=' + self.id, self.edit).then(response => {
        if (response.data.errno == 10000) {
          this.edit = {}
    ...
    remove (index) {
      let self = this
      self.$http(this.$config.BaseURL16 + '/staticpromotion/desktop/delete?id=' + self.data1[index].id).then(response => {
        if (response.data.errno == 10000) {
