          ...
          <Modal v-model="modalAdd" title="添加终端模板配置">
            ...
            <Form :model="info" label-position="right" :label-width="100">
              ...
              <FormItem label="配置mac:" v-if="info.configWay != 1">
                <Input v-model="info.mac" placeholder="请输入配置mac,按照,分隔..." type="textarea" :rows="4"></Input>
              </FormItem>
              <FormItem label="描述:">
                <Input v-model="info.description" placeholder="请输入描述..." type="textarea" :rows="4"></Input>
              </FormItem>
            </Form>
          </Modal>
        </FormItem>
...
<script>
  export default {
    ...
    methods: {
      // 列表
      getTableList() {
        this.$http.get(this.$config.BaseURL16 + '/scene/config/list?channelId=' + this.search.channelId + '&mac=' + this.search.mac + '&configWay=' + this.search.configWay + '&name=' + this.search.name +
      ...
      // 配置场景下拉框
      getSceneSelect() {
        this.$http.get(this.$config.BaseURL16 + '/scene/scene/list').then(response => {
          if (response.data.errno == 10000) {
      ...
      okAdd() {
        console.log(1111)
        this.$http.post(this.$config.BaseURL16 + '/scene/config/save', this.info).then(response => {
          if (response.data.errno == 10000) {
            this.info = {}
            this.modalAdd = false
            this.getTableList()
      ...
      okEdit() {
        ...
        this.$http.post(this.$config.BaseURL16 + '/scene/config/save', this.edit).then(response => {
          if (response.data.errno == 10000) {
      ...
      // 删除
      remove(row) {
        this.$http.get(this.$config.BaseURL16 + '/scene/config/delete?id=' + row.id).then(response => {
          if (response.data.errno == 10000) {
            this.getTableList()
