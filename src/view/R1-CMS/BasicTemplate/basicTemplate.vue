<template>
  <div id="template" class="layout-templateList">
    ...
    <Tabs type="card" @on-click='tabChange'>
        ...
        <Card style="width:100%;background-color: #eeeeee;">
          <div>
            <Row>
              ...
              <Col span="22">描述:{{tab.description}}</Col>
              <Button type="info" size="small" @click="DeskTopEditBtn(tab)" style="!important;">修改</Button>
              <!-- <Button type="error" size="small" @click='DesktopDelBtn(tab)'>删除</Button> -->
              <Poptip confirm title="确定要删除吗?" @on-ok="DesktopDelBtn(tab)" @on-cancel="cancel(tab)">
      ...
      <!-- <Button @click="handleTabsAdd1" size="small" slot="extra">删除</Button> -->
...
export default {
  ...
  methods: {
    ...
    getSelectList1 () {
      let self = this
      self.$http(this.$config.BaseURL16 + '/staticpromotion/template/list').then(response => {
        console.log(response)
    ...
    editDeskTopOk () {
      ...
      self.$http.post(`${this.$config.BaseURL16}/staticpromotion/desktop/save`, this.editDes).then(response => {
    ...
    getList () {
      ...
      self.$http.get(this.$config.BaseURL16 + '/staticpromotion/desktop/list?templateId=' + self.search.title).then(response => {
        console.log(response)
        if (response.data.errno == 10000) {
          // 给tab导航赋值
    ...
    tabChange (tab) {
      this.id = tab
      this.tabId = tab
      this.$http.get(this.$config.BaseURL16 + '/staticpromotion/desktop/getdesktopinfo?id=' + tab).then(response => {
        console.log(response)
        if (response.data.errno == 10000) {
    ...
    editTitleOk () {
      ...
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/desktop/setlayout', this.editTitle).then(response => {
        if (response.data.errno == 10000) {
          console.log(response)
          // this.editTitle = {};
    ...
    TabsAddOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/desktop/save', self.TabsAddInfo).then(response => {
        // if (response.data.errno == 10000) {
        self.TabsAddInfo = {}
        this.getList()
        self.TabsAdd = false
    ...
    saveDesktopInfo () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/desktop/savedata', {
        id: self.tabId,
        data: {
    ...
    DesktopDelBtn (tab) {
      let self = this
      self.$http(this.$config.BaseURL16 + '/staticpromotion/desktop/delete?id=' + tab.id).then(response => {
