  ...
  <div id="template" class="layout-templateList">
    <Form :model="search" label-position="right">
      <FormItem label="拓展模版名称">
        <Select v-model="search.title" style="width:160px" @on-change="getChange">
    ...
    <Modal v-model="DeskTopEdit" title="修改桌面">
      ...
      <Form :model="editDes" label-position="right" :label-width="100">
        ...
        <FormItem label="发布状态:">
            <Select style="width:150px" v-model="editDes.isRelease">
              <Option :value="1" :key="1">发布</Option>
              <Option :value="0" :key="0">未发布</Option>
            </Select>
          </FormItem>
    ...
    <div class="tmp-main" v-for="(item, index) in desktopinfo" :key="index"
      ...
      <div class="template1" v-if="item.template == 't1' && item.promotion.length > 0">
        ...
        <div class="xstv-temp1-leftImg" @click="infoModal(160,300,item.promotion[0],0,index)"
          ...
          <div style="position:relative;">
            ...
            <div style="position:absolute; z-index:2;top:0px;"
              ...
              <div v-if="item.promotion[0].isLock == 0"
                style="color: #ffffff;height: 30px;width: 300px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
        ...
        <div class="xstv-temp1-leftImg" @click="infoModal(160,300,item.promotion[1],1,index)"
        ...
        <div class="xstv-temp1-centerImg" @click="infoModal(160,300,item.promotion[2],2,index)"
        ...
        <div class="xstv-temp1-rightImg" v-if="item.promotion.length > 3">
          <div class="xstv-temp1-right top" @click="infoModal(160,300,item.promotion[3],3,index)"
          ...
          <div class="xstv-temp1-right bottom" @click="infoModal(160,290,item.promotion[4],4,index)"
            ...
            <div style="position:relative;">
              <img :src="item.promotion[4].imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'"
                width="290" height="150" alt="" style="margin-top: -30px;">
              <div style="position:absolute; z-index:2;top:-30px;"
                ...
                <div v-if="item.promotion[4].isLock == 0"
                  style="color: #ffffff;height: 30px;width: 280px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                  <span style="position: absolute;top: -2px;left: 15px;">
                    <Icon type="ios-unlock" />
                  </span>
                  <span>
      ...
      <div style="clear: both;"></div>
      <div class="template2" v-if="item.template == 't2' && item.promotion.length > 0">
        <!-- 标题 -->
        <div style="margin-top: 10px;">
          <p style="font-size: 14px;margin-left: 5px;float: left;">{{item.title}}</p>
          <p style="float: left;margin-left: 10px;margin-top:2px;">{{item.isHideTitle == 1 ? '隐藏' :
    ...
    <div style="margin-top: 30px;">
      <div style="clear: both;"></div>
      <Button @click="modalAdd = true;list_select_index=-1" style="margin-left:600px;">
          ...
          <div class="layout-layoutbucket-list">
            <RadioGroup v-model="list_select_index" style="width:100%;">
              <Card style="width:100%;background-color: #eeeeee;margin: 10px;" v-for="(item,index) in list"
               v-if='item._index!=0 && item._index!=4'>
                <Row>
                  <Col span="1">
                  <Radio :label="index">
                    <span></span>
                  ...
                  <div class="template">
                    ...
                    <div v-if="item._index == 1">
                      <p style="width:280px;height:160px;"></p>
                      <p style="width:280px;height:160px;"></p>
                      <p style="width:280px;height:160px;"></p>
                      <p style="width:280px;height:160px;"></p>
                    </div>
                    <!-- 模板2 -->
                     <div v-if="item._index == 2">
...
export default {
  ...
  methods: {
    ...
    onClickLock (item, promotionIndex, templateIndex) {
      ...
      self.$http(this.$config.BaseURL16 + '/custompromotion/desktop/lockpromotion?' + p).then(response => {
    ...
    getSelectList () {
      let self = this
      self.$http.get(this.$config.BaseURL16 + '/staticpromotion/action/list').then(response => {
    ...
    getSelectList1 () {
      let self = this
      self.$http(this.$config.BaseURL16 + '/custompromotion/template/list').then(response => {
        console.log(response)
    ...
    editDeskTopOk () {
      ...
      self.$http.post(`${this.$config.BaseURL16}/custompromotion/desktop/save`, this.editDes).then(response => {
        if (response.data.errno == 10000) {
    ...
    editOk () {
      ...
      self.$http.post(this.$config.BaseURL16 +
    ...
    tabChange (tab) {
      ...
      this.tabId = tab
      this.isRelease = tab
      this.$http.get(this.$config.BaseURL16 + '/custompromotion/desktop/getdesktopinfo?id=' + tab).then(response => {
        console.log(response)
        if (response.data.errno == 10000) {
    ...
    getList () {
      ...
      self.$http.get(this.$config.BaseURL16 + '/custompromotion/desktop/list?customTemplateId=' + self.search.title).then(response => {
        console.log(response)
        if (response.data.errno == 10000) {
          // 给tab导航赋值
    ...
    editTitleOk () {
      ...
      self.$http.post(this.$config.BaseURL16 + '/custompromotion/desktop/setlayout', this.editTitle).then(response => {
        if (response.data.errno == 10000) {
          console.log(response)
          // this.editTitle = {};
    ...
    TabsAddOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/custompromotion/desktop/save', self.TabsAddInfo).then(response => {
        if (response.data.errno == 10000) {
          this.TabsAddInfo = {}
          this.TabsAdd = false
          this.getList()
    ...
    saveDesktopInfo () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/custompromotion/desktop/savedata', {
        id: self.tabId,
        data: {
    ...
    getLayoutList () {
      ...
      let self = this
      self.$http(this.$config.BaseURL16 + '/staticpromotion/layoutbucket/list').then(response => {
        if (response.data.errno == 10000) {
    ...
    removeOk (tab) {
      let self = this
      self.$http(this.$config.BaseURL16 + '/custompromotion/desktop/delete?id=' + tab.id).then(response => {
    ...
    release (tab) {
      ...
      self.$http.post(this.$config.BaseURL16 + '/custompromotion/desktop/release', {
        id: self.tabId,
