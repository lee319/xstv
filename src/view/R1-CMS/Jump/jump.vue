...
export default {
  ...
  methods: {
    ...
    getTableList () {
      let self = this
      self.$http(this.$config.BaseURL16 + '/staticpromotion/action/list').then(response => {
    ...
    addOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/action/save', self.info).then(response => {
        if (response.data.errno == 10000) {
          this.info = {}
          this.modalAdd = false
          this.getTableList()
    ...
    editOk () {
      let self = this
      self.$http.post(this.$config.BaseURL16 + '/staticpromotion/action/save?id=' + self.id, self.edit).then(response => {
        if (response.data.errno == 10000) {
          this.edit = {}
    ...
    remove (index) {
      let self = this
      self.$http(this.$config.BaseURL16 + '/staticpromotion/action/delete?id=' + self.data1[index].id).then(response => {
        if (response.data.errno == 10000) {
          this.getTableList()
