  ...
  <div class="layout-administration">
    ...
    <div class="layout-administration-search">
      <Form :model="search" inline>
        ...
        <FormItem label="一级分类:" :label-width="70">
          <Select v-model="search.category_id" @on-change="changeSearchCategoryId" style="width:200px"
            placeholder="请选择一级分类...">
    ...
    <Modal v-model="modalAdd" title="添加应用" width="700">
      ...
      <Form :model="info" label-position="right" :label-width="130">
        <FormItem label="一级分类:">
          <CheckboxGroup v-model="info.category_id_list">
            <Checkbox v-for="item in CheckboxList1" :label="item.id" :value="item.check">{{item.name}}</Checkbox>
          </CheckboxGroup>
        </FormItem>
        <FormItem label="二级分类:">
          <CheckboxGroup v-model="info.category_sub_list">
            <div v-for="item in CheckboxList1">
              <p>{{item.name}}:</p>
              <Checkbox v-for="item1 in item.category_sub_info" :label="item1.id" :value="item1.check">{{item1.name}}
              </Checkbox>
            </div>
          </CheckboxGroup>
        </FormItem>
        ...
        <FormItem label="应用名称:">
          <Input v-model="info.name" placeholder="请输入应用名称..."></Input>
        </FormItem>
        <FormItem label="应用宝地址:">
          <Input v-model="info.web_url" placeholder="请输入应用宝地址..."></Input>
        </FormItem>
        <FormItem label="上传应用:">
          <input type="file" @change="ApplicationFileChange" accept=".apk">
        </FormItem>
        <FormItem label="应用包名:">
          <Input v-model="info.package" placeholder="请输入应用包名..."></Input>
        </FormItem>
        <FormItem label="应用icon:">
          <div class="demo-upload-list" v-for="item in uploadList3">
            <template v-if="item.status === 'finished'">
              ...
              <div class="demo-upload-list-cover">
                <Icon type="ios-eye-outline" @click.native="handleView4(item.name)"></Icon>
          ...
          <Upload ref="upload4" :show-upload-list="false" :on-success="handleSuccess4"
            :format="['jpg','jpeg','png','webp']" :max-size="2048" :on-format-error="handleFormatError4"
            :on-exceeded-size="handleMaxSize4" :before-upload="handleBeforeUpload4" multiple type="drag"
          ...
          <Modal title="查看图片" v-model="visible3">
            <img :src="showImgUrl3" v-if="visible3" style="width: 100%">
          </Modal>
        </FormItem>
        <FormItem label="版本名称:">
          <Input v-model="info.version" placeholder="请输入版本名称..."></Input>
        </FormItem>
        <FormItem label="版本code:">
          <Input v-model="info.version_code" placeholder="请输入版本code..."></Input>
        </FormItem>
        <FormItem label="应用大小:">
          <Input v-model="info.file_size" placeholder="请输入应用大小..."></Input>
        </FormItem>
        <FormItem label="上传截图:">
          <div class="demo-upload-list" v-for="item in uploadList4">
            <template v-if="item.status === 'finished'">
              <img :src="item.url">
              <div class="demo-upload-list-cover">
                <Icon type="ios-eye-outline" @click.native="handleView5(item.name)"></Icon>
          ...
          <Upload ref="upload5" :show-upload-list="false" :on-success="handleSuccess5"
            :format="['jpg','jpeg','png','webp']" :max-size="2048" :on-format-error="handleFormatError5"
            :on-exceeded-size="handleMaxSize5" :before-upload="handleBeforeUpload5" multiple type="drag"
            :action="$config.BaseURLfile + '/common/upload/file'" style="display: inline-block;width:58px;"
            :with-credentials="true">
            <div style="width: 58px;height:58px;line-height: 58px;">
              <Icon type="ios-camera" size="20"></Icon>
          ...
          <Modal title="查看图片" v-model="visible4">
            <img :src="showImgUrl4" v-if="visible4" style="width: 100%">
          </Modal>
        </FormItem>
        <FormItem label="图片类型:">
          <RadioGroup v-model="info.image_type">
            <Radio label="1">横图</Radio>
            <Radio label="2">竖图</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem label="内容是否付费:">
          <RadioGroup v-model="info.free">
            <Radio :label="0">免费</Radio>
            <Radio :label="1">付费</Radio>
            <Radio :label="2">免费+付费</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem label="是否有广告:">
          <!-- 是否有广告：0无、1有启动广告、2有浮窗广告、3有内推广告、4有启动和浮窗广告 -->
          <RadioGroup v-model="info.advert">
        ...
        <FormItem label="界面版式:">
          ...
          <RadioGroup v-model="info.screen">
        ...
        <FormItem label="适用年级:">
          <CheckboxGroup v-model="info.grade_info">
            <Checkbox v-for="item in Checkboxgrade" :label="item.id" :value="item.id">{{item.name}}</Checkbox>
          </CheckboxGroup>
        </FormItem>
        <FormItem label="适用人群:">
          <CheckboxGroup v-model="info.age_info">
            <Checkbox v-for="item in CheckboxCrowd" :label="item.id" :value="item.id">{{item.name}}</Checkbox>
          </CheckboxGroup>
        </FormItem>
        <FormItem label="品牌系列:">
          <Input v-model="info.brand" placeholder="请输入品牌系列..."></Input>
        </FormItem>
        <FormItem label="应用宝下载量（万）:">
          <Input v-model="info.downloads" placeholder="请输入应用宝下载量（万）..."></Input>
        </FormItem>
        <FormItem label="应用宝评分:">
          <Input v-model="info.score" placeholder="请输入应用宝评分..."></Input>
        </FormItem>
        <FormItem label="苹果商店点评数（万):">
          <Input v-model="info.apple_comments" placeholder="请输入苹果商店点评数（万）..."></Input>
        </FormItem>
        <FormItem label="苹果商店评分:">
          <Input v-model="info.apple_score" placeholder="请输入苹果商店评分..."></Input>
        </FormItem>
        <FormItem label="排序ID:">
          <Input v-model="info.sort_id" placeholder="请输入排序ID..."></Input>
        </FormItem>
        <FormItem label="开发商:">
          <Input v-model="info.developers" placeholder="请输入开发商..."></Input>
        </FormItem>
        <FormItem label="推荐话术:">
          <Input v-model="info.recommend_reason" type="textarea" :rows="4" placeholder="请输入推荐话术..."></Input>
        </FormItem>
        <FormItem label="权限信息:">
          <Input v-model="info.permission" type="textarea" :rows="4" placeholder="请输入权限信息..."></Input>
        </FormItem>
        <FormItem label="应用简介:">
          <Input v-model="info.description" type="textarea" :rows="4" placeholder="请输入应用简介..."></Input>
        </FormItem>
      </Form>
    </Modal>
    <!-- 应用信息列表 -->
    ...
    <Modal v-model="modalSee" title="查看" width="800">
      <Row>
        ...
        <Col span="8">
        应用icon:
        <img :src="news.version_info.big_icon" alt="" style="width:20px;height:20px;">
        </Col>
        <Col span="8">
    ...
    <Modal v-model="modalEdit" title="修改" width="700">
      ...
      <Form :model="edit" label-position="right" :label-width="130">
        <FormItem label="一级分类:">
          <CheckboxGroup v-model="edit.category_id_list">
            <Checkbox v-for="item in CheckboxList" :label="item.id" :value="item.check">{{item.name}}</Checkbox>
          </CheckboxGroup>
        </FormItem>
        <FormItem label="二级分类:">
        ...
        <FormItem label="类型标签:">
          <div style="float:left;">
            <Input v-model="edit.tagsList" placeholder="请输入类型标签..." style="width:400px;"></Input>
          </div>
          <Button type="primary" @click="updateType()">选择</Button>
        </FormItem>
        <FormItem label="应用名称:">
        ...
        <FormItem label="应用宝地址:">
          <Input v-model="edit.web_url" placeholder="请输入应用宝地址..."></Input>
        </FormItem>
        <FormItem label="内容是否付费:">
        ...
        <FormItem label="品牌系列:">
          <Input v-model="edit.brand" placeholder="请输入品牌系列..."></Input>
        </FormItem>
        <FormItem label="应用宝下载量（万）:">
        ...
        <Form :model="versionInfo" label-position="right" :label-width="100">
          ...
          <FormItem label="应用icon:">
            <Upload :action="$config.BaseURLfile + '/common/upload/file'" ref="upload" :on-success="handleSuccess1"
              :format="['jpg','jpeg','png','webp']" :max-size="2048" :on-format-error="handleFormatError1"
          ...
          <FormItem label="上传截图:">
            ...
            <Upload ref="upload" :show-upload-list="false" :on-success="handleSuccess"
              :format="['jpg','jpeg','png',webp]" :max-size="2048" :on-format-error="handleFormatError"
          ...
          <FormItem label="图片类型:">
            <RadioGroup v-model="versionInfo.image_type">
              <Radio label="1">横图</Radio>
              <Radio label="2">竖图</Radio>
            </RadioGroup>
          </FormItem>
      ...
      <Modal v-model="modalVersionEdit" title="修改">
        ...
        <Form :model="versionEdit" label-position="right" :label-width="100">
          ...
          <FormItem label="应用icon:">
            ...
            <Upload ref="upload3" :show-upload-list="defaultList2.length>0" :on-success="handleSuccess2"
              :format="['jpg','jpeg','png','webp']" :max-size="2048" :on-format-error="handleFormatError2"
              :on-exceeded-size="handleMaxSize2" :default-file-list="defaultList2" :before-upload="handleBeforeUpload2"
              multiple type="drag" :action="$config.BaseURLfile + '/common/upload/file'"
          ...
          <FormItem label="上传截图:">
            ...
            <Upload ref="upload3" :show-upload-list="defaultList1.length > 0" :on-success="handleSuccess3"
              :format="['jpg','jpeg','png','webp']" :max-size="2048" :on-format-error="handleFormatError3"
              :on-exceeded-size="handleMaxSize3" :default-file-list="defaultList1" :before-upload="handleBeforeUpload3"
              multiple type="drag" :action="$config.BaseURLfile + '/common/upload/file'"
              style="display: inline-block;width:58px;" :with-credentials="true">
              <div style="width: 58px;height:58px;line-height: 58px;">
                <Icon type="ios-camera" size="20"></Icon>
          ...
          <FormItem label="图片类型:">
            <RadioGroup v-model="versionEdit.image_type">
              <Radio :label="1">横图</Radio>
              <Radio :label="2">竖图</Radio>
            </RadioGroup>
          </FormItem>
  ...
  export default {
    ...
    methods: {
      ...
      handleSuccess2(res, file) {
        ...
        file.name = file.name;
        this.versionEdit.big_icon = res.data.url;
        this.uploadList2 = [file];
      },
      // 上传格式验证
      ...
      // 限制张数
      handleBeforeUpload2() {
        const check = this.uploadList2.length < 2
        if (!check) {
          this.$Notice.warning({
            title: '最多可上传1张图片！'
          })
      ...
      handleRemove3(file) {
        console.log(file)
        this.uploadList1 = this.uploadList1.filter(item => {
      ...
      handleView4(name) {
        this.imgName3 = name
        this.visible3 = true
        this.uploadList3.forEach(val => {
          if (val.name == name) {
            this.showImgUrl3 = val.url
          }
        })
      },
      // 删除
      ...
      handleView5(name) {
        this.imgName4 = name
        this.visible4 = true
        this.uploadList4.forEach(val => {
          if (val.name == name) {
            this.showImgUrl4 = val.url
          }
        })
      },
      // 删除
      ...
      // 限制张数
      handleBeforeUpload5() {
        const check = this.uploadList4.length < 5
        if (!check) {
          this.$Notice.warning({
            title: '最多可上传5张图片！'
          })
      ...
      // 应用修改一级分类
      getCheckboxList() {
        this.$http.get(this.$config.BaseURL11 + '/appstore/admin/v1/app/category/list?id=' + this.id).then(response => {
      ...
      // 应用添加一级分类
      getAddCheckboxList() {
        this.$http.get(this.$config.BaseURL11 + '/appstore/admin/v1/app/category/list').then(response => {
          console.log(response)
          if (response.data.errno == 10000) {
      ...
      addOk() {
        ...
        this.info.score *= 1
        this.info.apple_comments *= 1
        this.info.apple_score *= 1
        this.info.sort_id *= 1
        this.info.version_code *= 1
        this.info.image_type *= 1
        // // 上传icon
        // let imgUrls3 = []
        // this.uploadList3.forEach(val => {
        //   console.log(val)
        //   imgUrls3.push(val.url)
        // })
        this.info.big_icon = this.uploadList3[0].url

        // 上传截图
        let imgUrls4 = []
        this.uploadList4.forEach(val => {
          console.log(val)
          imgUrls4.push(val.url)
        })
        this.info.screen_shot = imgUrls4

        this.$http.post(this.$config.BaseURL11 + '/appstore/admin/v1/app/modify', this.info).then(response => {
          if (response.data.errno == 10000) {
            this.info = {}
            this.modalAdd = false
            this.getTableList()
      ...
      versionModify(row) {
        ...
        imageList.push({
          'name': 'name',
          'url': this.versionEdit.big_icon,
          'status': 'finished'
        })
        console.log(imageList)
        this.uploadList2 = imageList || []
      },
      // 版本管理修改确定按钮
      editVersionOk() {
        ...
        this.uploadList1.forEach(val => {
          console.log(val)
          imgUrls1.push(val.url)
        })
        this.versionEdit.screen_shot = imgUrls1
        this.versionEdit.image_type *= 1
        this.$http
          .post(
            this.$config.BaseURL11 + '/appstore/admin/v1/app/version/modify',
            this.versionEdit
    ...
    mounted() {
      ...
      this.uploadList3 = this.$refs.upload4.fileList

      this.uploadList4 = this.$refs.upload5.fileList
    }
  }
</script>