  ...
  <div id="template" class="layout-templateList">
    ...
    <div class="tmp-main" v-for="(item, index) in desktopinfo" :key="index" v-if="typeof item.promotion != 'undefined' && item.promotion.length > 0">
      ...
      <div style="clear: both;"></div>
        ...
        <div class="xstv-temp4-img" v-for="(promotionOne,indexP) in item.promotion" :key="indexP" @click="infoModal(74,750,promotionOne,indexP,index)">
          <div>
            <img :src="promotionOne.imgUrl" width="750" height="74" alt="">
          </div>
        </div>
      </div>
      <!-- 推荐位管理 -->
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        self.$http.get(this.$config.BaseURL + '/staticpromotion/desktop/list?templateId=' + self.search.title).then(response => {
          console.log(response)
          if (response.data.errno == 10000) {
            //给tab导航赋值
