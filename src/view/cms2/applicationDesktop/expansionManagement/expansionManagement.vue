<template>
  <div id="template" class="layout-templateList">
    <Form :model="search" label-position="right" inline>
      <FormItem label="拓展模版名称:" :label-width='120'>
    ...
    <Tabs type="card" @on-click='tabChange' v-model="tabName">
        ...
        <Card style="width:100%;background-color: #eeeeee;">
          <div>
            <Row>
              ...
              <Col span="6">来源:
              <span style="color: #69A341" v-if="tab.cp == 1">基础</span>
              <span style="color: #69A341" v-if="tab.cp == 0">扩展</span>
              </Col>
      ...
      <Button @click="handleTabsAdd" size="small" slot="extra">增加</Button>
       <Button size="small" slot="extra" @click='release' style='margin-left:5px'>{{releaseName}}</Button>
      <Modal v-model="TabsAdd" title="新建桌面">
        ...
        <Form :model="TabsAddInfo" label-position="right" :label-width="100" :rules="ruleValidate">
          ...
          <FormItem label="桌面类型:">
            <!-- <Input v-model="TabsAddInfo.type"></Input> -->
            <Select style="width:390px" v-model="TabsAddInfo.type">
         ...
         <FormItem label='年级-学期:'>
           <!-- <Input v-model='TabsAddInfo.grade'></Input> -->
           <Tree :data="data2" show-checkbox></Tree>
         </FormItem>
          <FormItem label="发布状态:">
            <Select style="width:390px" v-model="TabsAddInfo.isRelease">
    ...
    <!-- 修改基础桌面 -->
    <Modal v-model="DeskTopEdit" title="修改桌面" v-if='type==1'>
      <div slot="footer">
        <a @click="DeskTopEdit=false" class="btnSpan">取消</a>
        <Button @click="editDeskTopOk" type="primary">确定</Button>
      </div>
      <Form :model="editDes" label-position="right" :label-width="100" :rules="ruleValidate">
        <FormItem label="所属模版:">
          <Select style="width:390px" v-model="editDes.customTemplateId">
            <Option v-for="item in desktopTp" :value="item.id" :key="item.id">{{ item.title }}</Option>
          </Select>
        </FormItem>
        <FormItem label="桌面名称:">
        ...
        <FormItem label="桌面类型:">
            <!-- <Input v-model="TabsAddInfo.type"></Input> -->
          <Select style="width:390px" v-model="editDes.type">
            <Option :value='1'>基础</Option>
            <Option :value='2'>扩展</Option>
          </Select>
        </FormItem>
        ...
        <FormItem label="年级-学期:">
          <Input v-model="editDes.gradeDesc"></Input>
        </FormItem>
        <FormItem label='基础桌面状态:'>
          <Input></Input>
        </FormItem > 
        <FormItem label="发布状态:">
            <Select style="width:390px" v-model="editDes.isRelease">
              <Option :value="1" :key="1">发布</Option>
              <Option :value="0" :key="0">未发布</Option>
            </Select>
          </FormItem>
    ...
    <!-- 修改拓展桌面 -->
    <Modal v-model="DeskTopEdit" title="修改桌面" v-if='type==2'>
      <div slot="footer">
        <a @click="DeskTopEdit=false" class="btnSpan">取消</a>
        <Button @click="editDeskTopOk" type="primary">确定</Button>
      </div>
    ...
    <!-- 模版 -->
    <div class="tmp-main" v-for="(item, index) in desktopinfo" :key="index"
      v-if="typeof item.promotion != 'undefined' && item.promotion.length > 0">
      ...
      <div class="template1" v-if="item.template == 't1' && item.promotion.length > 0">
        ...
        <div class="xstv-temp1-leftImg" @click="infoModal(180,275,item.promotion[0],0,index)"
          @mouseenter="changeLock(item.promotion[0],true)" @mouseleave="changeLock(item.promotion[0],false)">
          <div style="position:relative;">
            <img :src="item.promotion[0].imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'"
              width="275" height="180" alt="">
            <div style="position:absolute; z-index:2;top:0px;"
              v-if="typeof item.promotion[0].lock != 'undefined' && item.promotion[0].lock == true"
              @click.stop="onClickLock(item.promotion[0],0,index)">
              <div v-if="item.promotion[0].isLock == 0"
                style="color: #ffffff;height: 30px;width: 275px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
        ...
        <div class="xstv-temp1-leftImg" @click="infoModal(180,275,item.promotion[1],1,index)"
          v-if="item.promotion.length > 1" @mouseenter="changeLock(item.promotion[1],true)"
          @mouseleave="changeLock(item.promotion[1],false)">
          <div style="position:relative;">
            <img :src="item.promotion[1].imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'"
              width="275" height="180" alt="">
            <div style="position:absolute; z-index:2;top:0px;"
              v-if="typeof item.promotion[1].lock != 'undefined' && item.promotion[1].lock == true"
              @click.stop="onClickLock(item.promotion[1],1,index)">
              <div v-if="item.promotion[1].isLock == 0"
                style="color: #ffffff;height: 30px;width: 275px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                <span style="position: absolute;top: -2px;left: 88px;">
                  <Icon type="ios-unlock" />
                </span>
        ...
        <div class="xstv-temp1-centerImg" @click="infoModal(180,130,item.promotion[2],2,index)"
          v-if="item.promotion.length > 2" @mouseenter="changeLock(item.promotion[2],true)"
          @mouseleave="changeLock(item.promotion[2],false)">
          <div style="position:relative;">
            <img :src="item.promotion[2].imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'"
              width="130" height="180" alt="">
            <div style="position:absolute; z-index:2;top:0px;"
              v-if="typeof item.promotion[2].lock != 'undefined' && item.promotion[2].lock == true"
              @click.stop="onClickLock(item.promotion[2],2,index)">
              <div v-if="item.promotion[2].isLock == 0"
                style="color: #ffffff;height: 30px;width: 130px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
        ...
        <div class="xstv-temp1-rightImg" v-if="item.promotion.length > 3">
          <div class="xstv-temp1-right top" @click="infoModal(82.5,130,item.promotion[3],3,index)"
            v-if="item.promotion.length > 3" @mouseenter="changeLock(item.promotion[3],true)"
            @mouseleave="changeLock(item.promotion[3],false)">
            <div style="position:relative;">
              <img :src="item.promotion[3].imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'"
                width="130" height="82.5" alt="">
              <div style="position:absolute; z-index:2;top:0px;"
                v-if="typeof item.promotion[3].lock != 'undefined' && item.promotion[3].lock == true"
                @click.stop="onClickLock(item.promotion[3],3,index)">
                <div v-if="item.promotion[3].isLock == 0"
                  style="color: #ffffff;height: 30px;width: 130px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                  <span style="position: absolute;top: -2px;left: 15px;">
                    <Icon type="ios-unlock" />
                  </span>
          ...
          <div class="xstv-temp1-right bottom" @click="infoModal(82.5,130,item.promotion[4],4,index)"
            v-if="item.promotion.length > 4" @mouseenter="changeLock(item.promotion[4],true)"
            @mouseleave="changeLock(item.promotion[4],false)">
            <div style="position:relative;">
              <img :src="item.promotion[4].imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'"
                width="130" height="82.5" alt="" style="margin-top: -30px;">
              <div style="position:absolute; z-index:2;top:-30px;"
                v-if="typeof item.promotion[4].lock != 'undefined' && item.promotion[4].lock == true"
                @click.stop="onClickLock(item.promotion[4],4,index)">
                <div v-if="item.promotion[4].isLock == 0"
                  style="color: #ffffff;height: 30px;width: 130px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                  <span style="position: absolute;top: -2px;left: 15px;">
                    <Icon type="ios-unlock" />
                  </span>
      ...
      <div class="template3" v-if="item.template == 't3' && item.promotion.length > 0">
        ...
        <div class="xstv-temp3-img" @click="infoModal(82.5,134,promotionOne,indexP,index)"
          v-for="(promotionOne,indexP) in item.promotion" :key="indexP" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
            <img :src="promotionOne.imgUrl" width="134" height="82.5" alt="">
            <div style="position:absolute; z-index:2;top:0px;"
              v-if="typeof promotionOne.lock != 'undefined' && promotionOne.lock == true"
              @click.stop="onClickLock(promotionOne,indexP,index)">
              <div v-if="promotionOne.isLock == 0"
                style="color: #ffffff;height: 30px;width: 134px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
          ...
          <div class="xstv-temp5-left" @click="infoModal(180,290,promotionOne,indexP,index)" v-if="indexP == 0"
            ...
            <div style="position:relative;">
              ...
              <div style="position:absolute; z-index:2;top:0px;"
                ...
                @click.stop="onClickLock(promotionOne,indexP,index)">
                <div v-if="promotionOne.isLock == 0"
                  style="color: #ffffff;height: 30px;width: 290px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
      ...
      <div class="template6" v-if="item.template == 't6' && item.promotion.length > 0">
        ...
        <div class="xstv-temp6-img" @click="infoModal(180,275,promotionOne,indexP,index)"
          v-for="(promotionOne,indexP) in item.promotion" :key="indexP" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
      ...
      <div class="template8" v-if="item.template == 't8' && item.promotion.length > 0">
        ...
        <div v-for="(promotionOne,indexP) in item.promotion" :key="indexP">
          ...
          <div class="xstv-temp1-centerImg" v-if="indexP > 0 && indexP < item.promotion.length-2"
            @click="infoModal(180,130,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
            @mouseleave="changeLock(promotionOne,false)">
            <div style="position:relative;">
              <img :src="promotionOne.imgUrl" width="130" height="180" alt="">
              <div style="position:absolute; z-index:2;top:0px;"
        ...
        <div class="xstv-temp1-rightImg" v-if="item.promotion.length > 4">
          ...
          <div class="xstv-temp1-right bottom" v-if="item.promotion.length > 5"
            @click="infoModal(82.5,130,item.promotion[5],5,index)" @mouseenter="changeLock(item.promotion[5],true)"
            @mouseleave="changeLock(item.promotion[5],false)">
            <div style="position:relative;">
              <img :src="item.promotion[5].imgUrl" width="130" height="82.5" alt="" style="margin-top: -30px;">
              <div style="position:absolute; z-index:2;top:-30px;"
                v-if="typeof item.promotion[5].lock != 'undefined' && item.promotion[5].lock == true"
                @click.stop="onClickLock(item.promotion[5],5,index)">
                <div v-if="item.promotion[5].isLock == 0"
                  style="color: #ffffff;height: 30px;width: 130px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                  <span style="position: absolute;top: -2px;left: 15px;">
                    <Icon type="ios-unlock" />
                  </span>
      ...
      <div class="template9" v-if="item.template == 't9' && item.promotion.length > 0">
        ...
        <div class="xstv-temp4-img" v-for="(promotionOne,indexP) in item.promotion" :key="indexP"
          @click="infoModal(82.5,134,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
            <img :src="promotionOne.imgUrl" width="134" height="82.5" alt="">
            <div style="position:absolute; z-index:2;top:0px;"
      ...
      <div class="template10" v-if="item.template == 't10' && item.promotion.length > 0">
        ...
        <div class="xstv-temp4-img" v-for="(promotionOne,indexP) in item.promotion" :key="indexP"
          @click="infoModal(180,134,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
      ...
      <div class="template12" v-if="item.template == 't12' && item.promotion.length > 0">
        ...
        <div v-for="(promotionOne,indexP) in item.promotion" :key="indexP">
          <div class="xstv-temp12-a" v-if="indexP == 0 || indexP == 3"
            @click="infoModal(180,275,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
            @mouseleave="changeLock(promotionOne,false)">
            <div style="position:relative;">
              <img :src="promotionOne.imgUrl" onerror="this.src='http://static.icloud.xstvos.cn/noPic.jpg'" width="275"
                height="180" alt="">
          ...
          <div class="xstv-temp12-b" v-if="indexP == 1 || indexP == 2"
            @click="infoModal(180,135,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
            @mouseleave="changeLock(promotionOne,false)">
            <div style="position:relative;">
              <img :src="promotionOne.imgUrl" width="135" height="180" alt="">
              <div style="position:absolute; z-index:2;top:0px;"
                ...
                @click.stop="onClickLock(promotionOne,indexP,index)">
                <div v-if="promotionOne.isLock == 0"
                  style="color: #ffffff;height: 30px;width: 135px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                  <span style="position: absolute;top: -1px;left: 20px;">
                    <Icon type="ios-unlock" />
                  </span>
                  <span>
      ...
      <div class="template14" v-if="item.template == 't14' && item.promotion.length > 0">
        ...
        <div class="xstv-temp14-img" v-for="(promotionOne,indexP) in item.promotion" :key="indexP"
          @click="infoModal(82.5,134,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
            ...
            <div style="position:absolute; z-index:2;top:0px;"
              ...
              @click.stop="onClickLock(promotionOne,indexP,index)">
              <div v-if="promotionOne.isLock == 0"
                style="color: #ffffff;height: 30px;width: 202.5px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                <span style="position: absolute;top: 0px;left: 20px;">
                  <Icon type="ios-unlock" />
                </span>
                <span>
      ...
      <div class="template16" v-if="item.template == 't16' && item.promotion.length > 0">
        ...
        <div style="clear: both;"></div>
        <div class="xstv-temp3-img" @click="infoModal(150,250,promotionOne,indexP,index)"
          v-for="(promotionOne,indexP) in item.promotion" :key="indexP" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
            ...
            <div style="position:absolute; z-index:2;top:0px;"
              ...
              @click.stop="onClickLock(promotionOne,indexP,index)">
              <div v-if="promotionOne.isLock == 0"
                style="color: #ffffff;height: 30px;width: 250px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px; ">
                <!-- <span style="position: absolute;top: 0px;left: 0px;">
                  <Icon type="ios-unlock" />
                </span> -->
                 <span style="position: absolute;top: -2px;left: 80px;">
                    <Icon type="ios-unlock" />
                  </span>
                <span>
                  点击锁定
              ...
              <div v-else
                style="color: #ffffff;height: 30px;width: 250px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                <span style="position: absolute;top:-2px;left: 65px;">
                  <Icon type="ios-lock" />
                </span>
                <span>
                  点击解除锁定
          ...
          <div class="xstv-temp1-bgWenzi3" style='width:250px;'>
            <p>{{promotionOne.title}}</p>
          </div>
          <!-- <div class="xstv-temp1-bgWenzi2" v-if="item.promotion.length > 4">
            <p>{{item.promotion[4].title}}</p>
      ...
      <div class="template17" v-if="item.template == 't17' && item.promotion.length > 0">
        ...
        <div class="xstv-temp17-img" v-for="(promotionOne,indexP) in item.promotion" :key="indexP"
          @click="infoModal(120,260,promotionOne,indexP,index)" @mouseenter="changeLock(promotionOne,true)"
          @mouseleave="changeLock(promotionOne,false)">
          <div style="position:relative;">
            ...
            <div style="position:absolute; z-index:2;top:0px;"
              ...
              @click.stop="onClickLock(promotionOne,indexP,index)">
              <div v-if="promotionOne.isLock == 0"
                style="color: #ffffff;height: 30px;width: 260px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                <span style="position: absolute;top: -2px;left: 80px;">
                  <Icon type="ios-unlock" />
                </span>
                <span>
              ...
              <div v-else
                style="color: #ffffff;height: 30px;width: 260px;background-color: #808695;font-size: 16px;text-align: center;line-height: 30px;">
                <span style="position: absolute;top: -2px;left:70px;">
                  <Icon type="ios-lock" />
                </span>
                <span>
                  点击解除锁定
      ...
      <Modal v-model="modalEdit" title="推荐位管理">
        ...
        <Form :model="edit" label-position="right" :label-width="100">
          ...
          <FormItem label="图片2:">
            <Upload :on-success="handleSuccess1" :with-credentials="true" :format="['jpg','jpeg','png']"
        ...
        <div class="layout-layoutbucket">
          <div class="layout-layoutbucket-list">
            <RadioGroup v-model="list_select_index" style="width:100%;">
              <Card style="width:100%;background-color: #eeeeee;margin: 10px;" v-for="(item,index) in list"
                v-if='item._index!=2 && item._index!=4'>
                <Row>
                  <Col span="1">
                  <Radio :label="index">
                    <span></span>
                  ...
                  <div class="template">
                    ...
                    <!-- 模板16 -->
                     <div v-if="item._index == 16">
                      <p style="width:220px;height: 150px;"></p>
                      <p style="width:220px;height: 150px;"></p>
                      <p style="width:220px;height: 150px;"></p>
                      <p style="width:220px;height: 150px;"></p>
                    </div>
...
<script>
  import './expansionManagement.less'
  export default {
    data() {
      return {
        //验证
    ...
    methods: {
      ...
      //修改桌面确定按钮
      // /staticpromotion/template/save
      editDeskTopOk() {
        let self = this;
        this.editDes.templateId = this.editDes.customTemplateId;
        self.$http.post(`${this.$config.BaseURL}/custompromotion/desktop/save`, this.editDes).then(response => {
          console.log(response,'桌面修改')
          if (response.data.errno == 10000) {
    ...
    // 修改基础桌面背景上传成功
      ...
      getTemplateName() {
        ...
        self.$http(this.$config.BaseURL + '/staticpromotion/template/list').then(response => {
          if (response.data.errno == 10000) {
            console.log(response,'模板名称')
            self.templateName = response.data.data.list;
            // 下拉框默认第一个
            console.log(this.templateName,'基础模板名称')
      ...
      modalAddOk() {
        ...
        console.log(this.desktopinfo);
        if (!Array.isArray(this.desktopinfo)) {
          // this.desktopinfo = ;
          let desktopinfo = [];
      ...
      removeOk(tab) {
        let self = this;               
        self.$http(this.$config.BaseURL + '/custompromotion/desktop/delete?id='+ tab.id).then(response => {
            console.log(response,'删除');
          if (response.data.errno == 10000) {
            console.log(response)
            // self.list = response.data.data;
            this.getList();
            this.tabName = this.tabs[0].id+'';
            // this.DesktopDel = false;
          }
        })
      ...
      release(tab){
        let self = this; 
        self.$http.post(this.$config.BaseURL + '/custompromotion/desktop/release', {
          id: self.tabId,
          isRelease:self.isRelease
        }).then(response => {
          // console.log(response,'发布')
          if (response.data.errno == 10000) {
            
            this.tabChange(self.tabId)
            this.$Message.success('发布成功!')
             this.releaseName = '取消发布';
            this.getList();
            this.tabName = this.tabs[0].id+'';
            this.tabChange(tab);
          }
