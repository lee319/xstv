...
<template>
    <div class="layout-group">
        ...
        <div class="layout-group-group">
            <Modal v-model="modalGroup" title="分组终端" width="1100" :transfer="false" @on-ok="okSearch"
                ...
                <div style="float: left;">
                    <Form :model="groupSearch" label-position="right" inline>
                        <FormItem label="终端厂商:" :label-width="80">
                            ...
                            <Select v-model="groupSearch.customerId" style="width:130px" placeholder="请选择终端厂商...">
                                <Option value="">全部</Option>
                                <Option v-for="item in cityList" :value="item._id" :key="item._id">{{item.customerName}}
                                </Option>
                            </Select>
                        </FormItem>
                        <FormItem label="商用型号:" :label-width="65">
                            <Select v-model="groupSearch.commercialId" style="width:130px" placeholder="请选择商用型号...">
                                <Option value="">全部</Option>
                        ...
                        <FormItem label="内部型号:" :label-width="80">
                            <Select v-model="groupSearch.insideId" style="width:130px" placeholder="请选择内部型号...">
                                <Option value="">全部</Option>
                        ...
                        <FormItem label="mac" :label-width="50">
                            <Input v-model="groupSearch.mac" style="width: 130px;" placeholder="请输入mac..."></Input>
                        </FormItem>
                        <FormItem>
            ...
            <Modal v-model="modalGroupAdd" title="添加" width="1100">
                <div slot="footer">
                    <a @click="addGroupCancel" class="span">取消</a>
                    <Button @click="addOk1" type="primary">确定</Button>
                </div>
                <div style="float: left;">
                    <Form :model="groupSearch1" label-position="right" inline>
                        <FormItem label="终端厂商:" :label-width="80">
                            <Select v-model="groupSearch1.customerId" style="width:160px" placeholder="请选择终端厂商...">
                                <Option value="">全部</Option>
                                <Option v-for="item in cityList" :value="item._id" :key="item._id">{{item.customerName}}
                                </Option>
                            </Select>
                        ...
                        <FormItem label="商用型号:" :label-width="65">
                            <Select v-model="groupSearch1.commercialId" style="width:160px" placeholder="请选择商用型号...">
                                <Option value="">全部</Option>
                                <Option v-for="item in cityList1" :value="item._id" :key="item._id">{{
                                    item.commercialName }}</Option>
                            </Select>
                        ...
                        <FormItem label="内部型号:" :label-width="80">
                            <Select v-model="groupSearch1.insideId" style="width:160px" placeholder="请选择内部型号...">
                                <Option value="">全部</Option>
                        ...
                        <FormItem label="mac" :label-width="50">
                            <Input v-model="groupSearch1.mac" style="width: 160px;" placeholder="请输入mac..."></Input>
                        </FormItem>
                        <FormItem>
                ...
                <div>
                    <Table border ref="selection" :columns="columns2" :data="data2"
                        @on-selection-change="data2SelectionChange"></Table>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            group(row) {
                let self = this;
                this.news = row;
                if (self.groupSearch.customerId == undefined) {
                    self.groupSearch.customerId = '';
                }
                if (self.groupSearch.insideId == undefined) {
                    self.groupSearch.insideId = '';
                }
                if (self.groupSearch.commercialId == undefined) {
                    self.groupSearch.commercialId = '';
                }
            ...
            getTerminalList() {
                let self = this;
                if (self.groupSearch1.customerId == undefined) {
                    self.groupSearch1.customerId = '';
                }
                if (self.groupSearch1.insideId == undefined) {
                    self.groupSearch1.insideId = '';
                }
                if (self.groupSearch1.commercialId == undefined) {
                    self.groupSearch1.commercialId = '';
                }
                self.$http(this.$config.BaseURL2 + '/tv/group/notGrouped?customerId=' + self.groupSearch1.customerId + '&commercialId=' + self.groupSearch1.commercialId + '&insideId=' + self.groupSearch1.insideId + '&mac=' + self.groupSearch1.mac + '&page=' + this.page1 + '&pageSize=' + this.pageSize1
                ).then(response => {
                    if (response.data.errno == 10000) {
                        console.log(response)
            ...
            addOk1() {
                ...
                this.GroupList.forEach((val, key) => {
                    deviceIds.push(val._id);
                });

                this.$http.get(`${this.$config.BaseURL2}/tv/group/addDevice?groupId=${this.news._id}&deviceId=${deviceIds.join(',')}`).then(response => {
                    console.log(response)
                    if (response.data.errno == 10000) {
                        this.modalGroupAdd = false;
                        this.group(this.news);
                        this.getTerminalList();
            ...
            // 选中data2的行
            data2SelectionChange(selection) {
