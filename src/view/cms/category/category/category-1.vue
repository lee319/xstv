                ...
                <Modal v-model="modalAdd" title="添加节目" :transfer="true" width="1200">
                    ...
                    <Form :model="search" label-position="right" inline>
                        <FormItem label="所属分类:" :label-width="70">
                            <Select style="width:200px" v-model="search.category_id">
                                <Option v-for="item in Taxonomy" :value="item.name" :key="item.id">{{ item.name }}</Option>
                            </Select>
                        </FormItem>
                        <FormItem label="名称:" :label-width="50">
                            <Input v-model="search.keywords" style="width:200px"></Input>
                        </FormItem>
                        <FormItem label="是否付费:" :label-width="80">
                    ...
                    <div class="layout-category-page">
                        <Page :total="total1" :current="1" :page-size="pageSize1" @on-change="changePage1"></Page>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            addOk() {
                ...
                self.$http.post(this.$config.BaseURL1 + '/media/recommend/add?list_id=' + ids).then(response => {
            ...
            getAddList() {
                let self = this;
                self.$http(this.$config.BaseURL1 + '/media/search?keywords=' + self.search.keywords + '&pageSize=' + self.pageSize1 + '&page=' + self.page1).then(response => {
                    console.log(response)
                    // 给表格赋值
