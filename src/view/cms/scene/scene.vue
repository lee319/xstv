...
<script>
    export default {
        ...
        methods: {
            ...
            // 场景列表
            getTableList() {
                this.$http(this.$config.BaseURL + '/scene/scene/list?name=' + this.sceneSearch.name + '&page=' + this.page + '&pageSize=' + this.pageSize).then(response => {
            ...
            //场景添加
            okSceneAdd() {
                this.$http.post(this.$config.BaseURL + '/scene/scene/save', this.sceneInfo).then(response => {
                    if (response.data.errno == 10000) {
                        this.sceneInfo = {};
                        this.getTableList();
            ...
            //修改
            okSceneEdit() {
                this.$http.post(this.$config.BaseURL + '/scene/scene/save', this.sceneEdit).then(response => {
                    if (response.data.errno == 10000) {
                        this.getTableList();
            ...
            remove(row) {
                this.$http.get(this.$config.BaseURL + '/scene/scene/delete?id=' + row.id).then(response => {
                    if (response.data.errno == 10000) {
                        this.getTableList();
            ...
            // 场景内容列表
            getContentList() {
                this.$http.get(this.$config.BaseURL + '/scene/content/list?sceneId=' + this.sceneId).then(response => {
                    if (response.data.errno == 10000) {
            ...
            getSelect() {
                this.$http.get(this.$config.BaseURL + '/custompromotion/template/list').then(response => {
                    if (response.data.errno == 10000) {
            ...
            getSelect1() {
                this.$http.get(this.$config.BaseURL + '/scene/type/list').then(response => {
                    if (response.data.errno == 10000) {
            ...
            okContentAdd() {
                ...
                this.$http.post(this.$config.BaseURL + '/scene/content/save', this.ContentInfo).then(response => {
                    if (response.data.errno == 10000) {
                        this.ContentInfo = {};
                        this.getContentList();
            ...
            okContentEdit() {
                ...
                this.$http.post(this.$config.BaseURL + '/scene/content/save', this.ContentEdit).then(response => {
                    if (response.data.errno == 10000) {
                        this.getContentList();
            ...
            // 内容维护删除
            remove1(row) {
                this.$http.get(this.$config.BaseURL + '/scene/content/delete?id=' + row.id).then(response => {
                    if (response.data.errno == 10000) {
                        this.getContentList();
                        this.$Message.success('删除成功！')
                    }
