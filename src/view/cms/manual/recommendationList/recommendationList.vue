...
<template>
    <div class="layout-category">
                ...
                <Modal v-model="modalAdd" title="添加节目" :transfer="true" width="1200">
                    ...
                    <Form :model="search" label-position="right" inline>
                        ...
                        <FormItem label="名称:" :label-width="50">
                            <Input v-model="search.title" style="width:200px" placeholder="请输入名称搜索..."></Input>
                        </FormItem>
        ...
        <Modal v-model="modalEdit" title="修改">
            <div slot="footer">
                <a @click="modalEdit1" class="span">取消</a>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getTabList() {
                let self = this;
                self.$http(this.$config.BaseURL18 + '/cms/recommend/lists').then(response => {
                    if (response.data.errno == 10000) {
                        console.log(response,'推荐列表')
            ...
            //内容管理列表
            getContentList() {
                // /cms/recommend/detail
                let self = this;
                self.$http(this.$config.BaseURL18 + '/cms/recommend/detail?list_id=' + self.listId + '&pageSize=' + self.pageSize + '&page=' + self.page).then(response => {
            ...
            addOk() {
                ...
                self.$http.post(this.$config.BaseURL18 + '/cms/recommend/add?list_id=' + self.listId, postData).then(response => {
                    if (response.data.errno == 10000) {
                        self.modalAdd = false;
                        this.getContentList()
            ...
            //修改确定
            editOk() {
            //    /cms/recommend/videoreorder
                let self = this;
                self.$http.post(this.$config.BaseURL18 + '/cms/recommend/videoreorder?list_id=' + self.listId + '&video_id=' + self.videoId1 + '&new_order=' + self.edit.recommend_order, self.edit, {
            ...
            getAddList() {
                ...
                self.$http(this.$config.BaseURL18 + '/cms/media/search?title=' + self.search.title + '&pageSize=' + self.pageSize1 + '&page=' + self.page1 + '&category_id=' + self.search.category_id + '&cost_type=' + self.search.cost_type).then(response => {
                    if (response.data.errno == 10000) {
            ...
            //删除
            remove(row) {
                // /cms/recommend/remove
                let self = this;
                self.$http.post(this.$config.BaseURL18 + '/cms/recommend/remove?list_id=' + self.listId + '&video_id=' + row.id).then(response => {
                    if (response.data.errno == 10000) {
