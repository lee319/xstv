<style>
    ...
    .layout-default .ivu-poptip-confirm .ivu-poptip-body .ivu-icon {
        font-size: 16px;
        color: #ff9900;
        line-height: 18px;
        position: absolute;
...
<template>
    <div class="layout-default">
        <div class="layout-default-search">
        ...
        <div class="layout-default-table">
...
<script>
    export default {
        ...
        methods: {
            // 表格数据
            getTableList() {
                this.$http(this.$config.BaseURL1 + '/media/v2/priority/default/list?src_name=' + this.search.src_name + '&page=' + this.page + '&pageSize=' + this.pageSize).then(response => {
            ...
            //修改确定按钮
            okEdit() {
                this.$http.post(this.$config.BaseURL1 + '/media/v2/priority/default/edit?src_id=' + this.id, this.edit).then(response => {
                    if (response.data.errno == 10000) {
