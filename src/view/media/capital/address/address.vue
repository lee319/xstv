...
<template>
    <div class="layout-address">
        ...
        <div class="layout-address-search">
            <Form :model="edit1" inline>
                <FormItem label="节目集名称:" :label-width="80">
                    <Input v-model="edit1.title" style="width: 200px;" disabled></Input>
                </FormItem>
            </Form>
        </div>
        <!-- 添加 -->
        <div class="layout-address-add">
            ...
            <Modal v-model="modalAdd" title="添加" width="auto" :styles="{top: '20px'}" @on-ok="addOk">
                <Form :model="info" label-position="right" :label-width="100">
                    <FormItem label="节目集名称:">
                        <Input v-model="edit1.title" disabled></Input>
                    </FormItem>
                    <FormItem label="海报比例:">
            ...
            <Modal v-model="modalAddCut" title="裁剪图片" fullscreen @on-ok="modalCutOnOk">
                <div style="width:100%;height:100%;">
                    <vue-cropper v-if="modalAddCut == true" ref="cropper" :img="option.img" :output-size="option.size"
                        ...
                        :center-box="option.centerBox" @realTime="realTime" :high="option.high" @img-load="imgLoad"
                        :fixedNumber="option.fixedNumber" :fixed="option.fixed"></vue-cropper>
                </div>
            </Modal>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            getTableList() {
                ...
                self.$http(this.$config.BaseURL1 + '/media/v2/poster/address/list?albumId=' + this.search.albumId + '&page=' + self.page + '&pageSize=' + self.pageSize ).then(response => {
        ...
        mounted() {
            // 读取修改海报信息
            this.edit1 = this.$route.query.edit || {};
            this.search = this.edit1;
            this.search.albumId = this.edit1.id;
            this.search.name = this.edit1.title;
            this.getTableList();
