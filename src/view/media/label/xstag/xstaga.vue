            ...
            <Drawer title="top排行" :closable="false" v-model="value1" width="600">
                <div style="float:right;">
                    ...
                    <Modal v-model="modalDetails" title="top榜" width="1100">
                        ...
                        <div style="float:right">
                            <Button type="success" @click="showModalAdd">添加</Button>
                            <Modal v-model="modalAdd" title="添加" @on-ok="okAdd">
                                <Form :model="info" label-position="right" :label-width="100">
                                    <FormItem label="标签:">
                                        <Input v-model="info.tag_name" disabled></Input>
                                    </FormItem>
                                    <FormItem label="节目分类:">
                                        <Input v-model="info.category_name" disabled></Input>
                                    </FormItem>
                                    <FormItem label="节目名称:">
                        ...
                        <Modal v-model="modalEdit" title="修改" @on-ok="okEdit">
                            <Form :model="edit" label-position="right" :label-width="100">
                                <FormItem label="标签:">
                                    <Input v-model="edit.tag_name" disabled></Input>
                                </FormItem>
                                <FormItem label="节目分类:">
    ...
    export default {
        ...
        methods: {
            ...
            getProductList() {
                ...
                this.$http(this.$config.BaseURL1 + "/media/v2/complete/program/list?title=" + this.searchProgram.title + '&directors=' + this.searchProgram.directors + '&page=' + this.page + '&pageSize=' + this.pageSize + '&categoryId=' + this.tabId).then(response => {
                    if (response.data.errno == 10000) {
                        console.log(response);
            ...
            modify(row) {
                ...
                this.edit.tag_id = this.aaa.data.aid;
                this.edit.tag_name = this.aaa.topic;
                this.edit.category_id=this.tabId;
                this.edit.category_name=this.tabName;
            ...
            showModalAdd(){
                ...
                this.info.tag_name = this.aaa.topic;
                this.info.category_id=this.tabId;
                this.info.category_name=this.tabName;
            }
        },
        mounted() {
