        ...
        <div class="layout-order-search">
            <Form :model="search" label-position="right" inline>
                ...
                <FormItem label="创建时间:" :label-width="70">
                   <DatePicker type="daterange" placement="bottom-end" placeholder="请选择请求时间..." style="width: 200px"
                ...
                <FormItem label="商品类型:" :label-width="70">
                    <Select v-model="search.goodsClass" style="width:200px" aria-placeholder="请选择商品类型...">
                        <!-- <Option value="">全部</Option>
                        <Option :value="1">点播视频</Option>
                        <Option :value="2">影视会员</Option> -->
                        <Option v-for="item in OptionsList" :value="item.id">{{item.name}}</Option>
                    </Select>
                </FormItem>
                <FormItem>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            //导出excel
            downloadExcel() {
            // window.open(this.$config.BaseURL10 + '/tv/export/boardactivationlog', '_blank')
                let self = this;
                //开始时间
                let createTimeStart = moment(self.timer[0]).format("X");
                //结束时间
                ...
                window.open(this.confing.BaseURL10 + '/order/export?userId=' + self.search.userId + '&orderId=' + self.search.orderId 
